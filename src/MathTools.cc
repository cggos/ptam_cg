...
void MathTool::LHFromRH(SE3<> rhm,SE3<> &lhm) 
{ 
...
float* MathTool::se3_to_float(SE3<> tform) 
{
...
void MathTool::se3_to_float(SE3<> tform, float* mat) 
{
    Matrix<3,3,double> rot = tform.get_rotation().get_matrix();
    Vector<3,double> trans = tform.get_translation();
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j)
            mat[i*4+j] = (float)rot(i,j); // to row-major matrix

    mat[3]  = (float)trans[0];
    mat[7]  = (float)trans[1];
    mat[11] = (float)trans[2];
}

void MathTool::se3_to_euler(SE3<> tform, float* mat)
{
    Matrix<3,3,double> R = tform.get_rotation().get_matrix();
    Vector<3,double> trans = tform.get_translation();//R.T()*tform.get_translation();
    mat[0] = trans[0];
    mat[1] = trans[1];
    mat[2] = trans[2];
    ...
    if(sy > 1E-6)
    {
        mat[3] = atan2(R(2,1),R(2,2));
        mat[4] = atan2(-R(2,0),sy);
        mat[5] = atan2(R(1,0),R(0,0));
    }
    else
    {
        mat[3] = atan2(-R(1,2),R(1,1));
        mat[4] = atan2(-R(2,0),sy);
        mat[5] = 0;
    }
#else
    mat[3] = atan2(R(2,1), R(2,2));
    mat[4] = atan2(-R(2,0), sqrt(R(2,1)*R(2,1) + R(2,2)*R(2,2)));
    mat[5] = atan2(R(1,0), R(0,0));
#endif
}

void MathTool::SE3_to_RM(SE3<> se3, float* rm) 
{
    Matrix<3,3,double> rot = se3.get_rotation().get_matrix();
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j)
            rm[i*3+j] = (float)rot(i,j); // to column-major matrix
}
 ...
 */
void MathTool::Quaternion2Euler(float *quater, float **euler)
{
    float x = quater[0];
    float y = quater[1];
    float z = quater[2];
...
void MathTool::Euler2Quaternion(float *euler, float **quater)
{
  ...
  * *euler[3] : theta_x, theta_y, theta_z
  */
void MathTool::RotationMatrix2Euler(float *rm, float **euler)
{
    float rm11 = rm[0];
    float rm21 = rm[3];
    float rm31 = rm[6];
...
void MathTool::RotationMatrixEnhanced2Euler(float *rme, float **euler)
{
    float rm11 = rme[0];
    float rm21 = rme[4];
    float rm31 = rme[8];
...
void MathTool::ICP_SVD(const std::vector<Vector<3> > &ptsA, const std::vector<Vector<3> > &ptsB, SE3<> &se3AfromB)
{
    ...
    int determinant = W(0,0)*W(1,1)*W(2,2) + W(0,1)*W(1,2)*W(2,0) + W(0,2)*W(1,0)*W(2,1) -
            (W(0,0)*W(1,2)*W(2,1) + W(0,1)*W(1,0)*W(2,2) + W(0,2)*W(1,1)*W(2,0));

    //svd decomposition
    SVD<3,3> svd(W);
    Matrix<3> rotation = TooN::Identity;
    Vector<3> translation = TooN::Zeros;
    if(determinant != 0)
    {
        rotation = svd.get_U()*svd.get_VT();
    }
    translation = ptCenterA-rotation*ptCenterB;

    //compose SE3
    se3AfromB.get_rotation() = rotation;
    se3AfromB.get_translation() = translation;
}

// ICP using Quarternion Decomposition
void MathTool::ICP_QD(const std::vector<Vector<3> > &ptsA, const std::vector<Vector<3> > &ptsB, SE3<> &se3AfromB)
{
    unsigned int nSizeA = ptsA.size();
    unsigned int nSizeB = ptsB.size();
